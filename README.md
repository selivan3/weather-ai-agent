...
Учебный проект: Магистратура КубГУ, «Прикладная информатика», профиль «Искусственный интеллект и машинное обучение», 2025–2027. Автор — Иван Селиванов.
...
**Приветствие агента и виджет текущей погоды.** Агент сразу показывает карточку «Сейчас» по дефолтному городу (Краснодар) с follow-up кнопками («Ещё 3 дня», «Зонт?»).

![Chat interface — welcome + current weather](docs/screenshots/01-chat-interface.png)

**Ответ на вопрос пользователя с виджетом прогноза.** На запрос «есть ли сегодня дождь?» агент подтянул прогноз и отрисовал карточку «Прогноз на сегодня»: «Местами дождь», вероятность дождя 99%, детализация по средней/утру/вечеру + follow-up'ы («Сравнить день», «Когда гулять?»).
